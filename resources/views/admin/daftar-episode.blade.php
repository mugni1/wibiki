<x-admin-layout>
    ...
    <section class="px-5 w-full mx-auto pb-5">
        <form class=" w-full flex flex-wrap" action="" method="GET">
            <input class=" w-10/12 border py-2 px-3 rounded-s-md md:w-11/12" type="text" name="keywords" placeholder="Search of name ..." id="">
    ...
    <section class="px-5 w-full pb-5">
        ...
        <div class=" w-full flex flex-wrap justify-between items-center my-5 py-2">
            ...
            <div class=" w-full md:w-4/12 font-semibold">
                <p>Halaman {{ $videos->currentPage() }} dari {{ $videos->lastPage() }}.</p>
            </div>
            <div class=" w-full md:w-4/12">
                <p class=" w-full">Menampilkan {{ $videos->count() }} dari {{ $videos->total() }} postingan</p>
            </div>
        </div>
